...
func (s *RepairService) UpdateByTechnician(repairID uint, techID uint, req *UpdateRepairRequest) (*RepairDTO, error) {
	// First verify the repair belongs to this technician
	repair, err := s.repo.FindByID(repairID)
	if err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, ErrRepairNotFound
		}
	...
	// Verify technician owns this repair
